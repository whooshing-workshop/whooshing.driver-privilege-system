...
public struct RoleAppointmentGuard: AsyncMiddleware {
    public let system: PrivilegeSystem
    
    public init(system: PrivilegeSystem) {
        self.system = system
    }

    public func respond(to request: Request, chainingTo next: AsyncResponder) async throws -> Response {
        return try await run(to: request, chainingTo: next)
    }
    
    private func run(to request: Request, chainingTo next: AsyncResponder) async throws(PrivilegeSystemErrcase.ErrType) -> Response {
        // ApiValidator 成功后会 `request.auth.login(authData)`；若拿不到，说明链路配置有误，直接拒绝
        guard let auth = request.auth.get(AuthData.self) else {
            throw PrivilegeSystemErrcase.roleValidateFailed.d("用户身份尚未认证，RoleAppointmentGuard 必须挂在 ApiValidator 之后", category: .internal)
        }

        let roleId = auth.role.id
        let userId = auth.token.user.id

        let appointed = try await required(throws: PrivilegeSystemErrcase.roleValidateFailed, "校验角色任命关系失败", category: .internal) {
            try await system.role.is(roleId: roleId, appointedTo: userId)
        }
        ...
        guard appointed else {
            ...
            throw PrivilegeSystemErrcase.roleValidateFailed.d("所声明的角色(X-Role-Id)并未任命给当前用户", category: .internal)
        }
